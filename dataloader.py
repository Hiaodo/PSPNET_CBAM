...
class VOCDataset(Dataset):
    def __init__(self, root, split='train',num_classes=20, base_size=True, augment=True,
                 crop_size=713, scale=True, flip=True, rotate=False, blur=True,):
        super(VOCDataset, self).__init__()
        self.root = root     # 存放数据集的根路径
        self.num_classes = num_classes  # 数据集的类别总数
        self.MEAN = [0.28802046, 0.3264235, 0.28579238] # 数据集的均值和方差
        self.STD = [0.1874096, 0.19018781, 0.1871926]   # 已改cityscapes的，上同
    ...
    def _set_files(self):
        self.root = os.path.join(self.root)  # VOC数据集的路径
        self.image_dir = os.path.join(self.root, 'JPEGImages') # 图片的存放路径
        self.label_dir = os.path.join(self.root, 'mask') # 标签的存放路径
